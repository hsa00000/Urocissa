...
pub fn process_image_info(
    ...
    fix_image_width_height(database, &exif_vec);

    // Adjust orientation if required
    fix_image_orientation(&exif_vec, &mut dynamic_image);
    ...
    if let Some(ts) = compute_timestamp_ms_by_exif(&exif_vec) {
...
pub fn process_video_info(database: &mut DatabaseSchema) -> Result<BTreeMap<String, String>> {
    // Extract EXIF‑like metadata via ffprobe
    let exif_vec = generate_exif_for_video(database)
    ...
    fix_video_width_height(database, &exif_vec);
