...
static TREE_SNAPSHOT_IN_DISK: LazyLock<redb::Database> = LazyLock::new(|| {
    ...
    let path = db_directory.join("temp_db_v7.redb");
    if let Some(parent) = path.parent()
        && !parent.exists()
    {
        std::fs::create_dir_all(parent).unwrap();
    }
    // Tree/query snapshots are derived data. Schema 7 adds independent tree
    // identity and selection epochs, so old files can be discarded without a
    // durable migration.
    for legacy_path in [
        db_directory.join("temp_db.redb"),
        db_directory.join("temp_db_v4.redb"),
        db_directory.join("temp_db_v5.redb"),
        db_directory.join("temp_db_v6.redb"),
    ] {
