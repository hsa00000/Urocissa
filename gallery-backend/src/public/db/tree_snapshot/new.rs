...
static TREE_SNAPSHOT_IN_DISK: LazyLock<redb::Database> = LazyLock::new(|| {
    ...
    let path = db_directory.join("temp_db_v5.redb");
    if let Some(parent) = path.parent() {
        if !parent.exists() {
            std::fs::create_dir_all(parent).unwrap();
        }
    }
    // Tree/query snapshots are derived data. Schema 5 adds compact scrollbar
    // metadata beside the SlotRef tables, so old files can be discarded.
    for legacy_path in [
