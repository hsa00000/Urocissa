...
pub fn fix_image_orientation(database: &DatabaseWithTag, dynamic_image: &mut DynamicImage) -> () {
...
pub fn fix_image_width_height(database: &mut DatabaseWithTag) -> () {
    if let Some(orientation) = database.exif_vec.get("Orientation") {
        match orientation.as_str() {
            "row 0 at right and column 0 at top" => {
...
pub fn fix_video_width_height(database: &mut DatabaseWithTag) -> () {
