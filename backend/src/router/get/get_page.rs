...
pub static INDEX_HTML: LazyLock<String> = LazyLock::new(|| {
    fs::read_to_string("../frontend/dist/index.html").expect("Unable to read index.html")
});
...
pub async fn favicon() -> Option<NamedFile> {
    NamedFile::open(Path::new("../frontend/dist/favicon.ico"))
        .await
        .ok()
}
...
pub async fn sregister_sw() -> Option<NamedFile> {
    NamedFile::open(Path::new("../frontend/dist/registerSW.js"))
        .await
        .ok()
}
...
pub async fn service_worker() -> Option<NamedFile> {
    NamedFile::open(Path::new("../frontend/dist/serviceWorker.js"))
        .await
        .ok()
}
