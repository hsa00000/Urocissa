...
The artifact schema is version 6 and includes the complete RAM-first edit and write-behind workload plus the selected 8,192-record flush chunk size in `environment.workload`. Derived tree/query snapshots also use internal schema 6: one fixed-width ordinal/selection bitmap blob per tree snapshot plus cached scrollbar boundaries and structural-epoch validation. Incompatible temp/query caches are deleted at startup. Older benchmark baselines are intentionally incompatible with comparison runs. Generate a fresh baseline after upgrading:
