...
The artifact schema is version 5 and includes the complete RAM-first edit and write-behind workload plus the selected 8,192-record flush chunk size in `environment.workload`. Derived tree/query snapshots also use internal schema 5: compact `SlotRef` tables plus cached scrollbar boundaries. Incompatible temp/query caches are deleted at startup. Older benchmark baselines are intentionally incompatible with comparison runs. Generate a fresh baseline after upgrading:
